...
EXP_CONFIG_PATH = os.path.join(home_path, "3-config-json/exp_default/config_default_pps_habituation_ALEX.json")
default_exp_config = load_json(EXP_CONFIG_PATH)
config_all = deep_update(hardware_config, default_exp_config)

ANIMAL_CONFIG_PATH = os.path.join(home_path,f"3-config-json/subject_exp/{mouse_name}/config_{mouse_name}_pps_habituation_ALEX.json")
...
try:
    ...
    while True:
        if new_ball == True and center_reached:
        ...
        for bi, ball in enumerate(balls):
            ...
            if center_reached:
                ball['y_cm'] -= 40.0 * dt_clamped #40.0 is velocoity
           ...
           # bottom_threshold = -half_screen_h + CIRCLE_RADIUS_CM
            ...
            drawn_x = world_to_screen_x(ball['world_x_cm'], win_start, SCREEN_WIDTH_CM, SPACE_WIDTH_CM)
            if abs(drawn_x)<ball['radius'] and horiz_in:
                # print("center_reached")
                center_reached = True
            ...
            if ball['y_cm'] <= bottom_threshold:
                ...
                if horiz_in and center_reached and (not reward_sent_this_frame):
                    match REWARD_FUNCTION:
                        case "list":
