...
def find_latest_next_config(home_path, exp_name, mouse_name, raw_data_dir=None):
    ...
    search_roots = [
        ...
        # Temporary folders created by database-mode sessions.
...
if USE_DATABASE:
    ...
    local_data_save_dir = Path.home() / "Desktop"
    storage = SessionStorageManager(
        local_base_dir = local_data_save_dir,
...
if USE_DATABASE:
    try:
        ...
        while True:
            use_local_config = input("Could not retrieve parameters from database.Do you want to continue with use local parameters [y/n]:").strip().lower()
            if use_local_config in {"y"}:
                break
            elif use_local_config in {"n"}:
                sys.exit()
...
LATEST_NEXT_CONFIG_PATH      =  os.path.join(session_dir, f"exp_config_nextsession_{exp_name}_{mouse_name}_{date_str}_{current_time}.json")
BEHAV_RESULTS_FILENAME      =  os.path.join(session_dir, f"behav_results_{exp_name}_{mouse_name}_{date_str}_{current_time}.json")
