...
def build_smb_link(
    ...
    return to_windows_path(
        f"{base}\\{experiment_name}\\{mouse_name}\\{foldername}"
    )


def paths_are_same(path_a: str | Path, path_b: str | Path) -> bool:
    """Compare paths safely on Windows, including case-insensitive drive paths."""
    a = os.path.normcase(os.path.abspath(str(path_a)))
    b = os.path.normcase(os.path.abspath(str(path_b)))
    return a == b
...
class SessionStorageManager:
    ...
    def get_next_session_num(
        ...
        """Return the next unused numeric session ID from existing folders."""
        session_nums: list[int] = []
        roots = {
            self.local_base_dir / experiment_name / mouse_name,
            self.nas_base_dir / experiment_name / mouse_name,
        }
        pattern = re.compile(r"^Session\s+(\d+)(?:_|\s*-\s*)")
        ...
        for root in roots:
            ...
            try:
                ...
                print(f"Warning: failed scanning session folders under {root}: {e}")
    ...
    def make_folders(
        ...
        *,
        ...
        foldername = f"Session {session_num}_{today_str}"
...
class SessionManager:
    ...
    def begin_session(
        ...
        *,
        ...
        smb_link = build_smb_link(
            self.storage.smb_base_unc,
            experiment_name,
            mouse_name,
            foldername,
        )
    ...
    def finalize_session(
        ...
        *,
        ...
        direct_write = paths_are_same(ctx.local_dir, ctx.nas_dir)

        if transfer_to_nas and not direct_write:
            ...
            try:
                ...
                delete_local = False
        elif transfer_to_nas and direct_write:
            print(f"[storage] Data were written directly to: {ctx.nas_dir}")

        if delete_local and not direct_write:
            try:
