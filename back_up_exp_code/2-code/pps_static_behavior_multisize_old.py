...
RUNTIME_TIMEOUT_MINUTES = 60.0  # runtime timeout in minutes (None or <=0 disables)
...
CIRCLE_RADIUS_CM_LIST = [3.0, 1.5]
BALL_FALL_SPEED_CM_S = 30.0        # default downward speed (cm / second) used if region.linear_velocity is None
#SUCCESS_EDGE_TOLERANCE_MULT = 3.5 # multiplier of circle radius for success tolerance defualt(?) = 1.25 
SUCCESS_EDGE_TOLERANCE_RANGE = [-9.0, 9.0]
...
SPAWN_GAUSS_CENTER_LIST = [-15.0, -10.0, 10.0, 15.0]
...
SPAWN_Y_OFFSET =  50.0 # By Shizhao Liu 03/06/2026. Make the ball start closer to the animal
...
class ExperimentConfig:
    ...
    CIRCLE_RADIUS_CM_LIST: tuple[float, float]
    BALL_FALL_SPEED_CM_S: float
    SUCCESS_EDGE_TOLERANCE_RANGE: tuple[float, float]
...
exp_config = ExperimentConfig(
    ...
    CIRCLE_RADIUS_CM_LIST = CIRCLE_RADIUS_CM_LIST,
    BALL_FALL_SPEED_CM_S = BALL_FALL_SPEED_CM_S,
    SUCCESS_EDGE_TOLERANCE_RANGE = SUCCESS_EDGE_TOLERANCE_RANGE,
...
try:
    ...
    while True:
        ...
        if len(balls) > 0:
            ...
            slot_cells[0] = f"{sid}|{rel_deg:.1f}|{ball_bl_y:.3f}|1|{ball_radius:.1f}"
