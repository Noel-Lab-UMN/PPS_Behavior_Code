...
from util_analysis import analyze_pps_newHabituation,  update_params_pps_newHabituation
...
def write_json_atomic(filename, data):
    """Write JSON without risking a half-written latest-config file."""
    path = Path(filename)
...
def find_latest_next_config(home_path, exp_name, mouse_name, raw_data_dir=None):
    """
    Find the most recent locally accessible next-session config.

    New sessions use a stable per-mouse file that is independent of whether
    the database is enabled. Legacy session folders are searched only when
    that stable file does not exist yet.
    """
    behavior_mouse_root = (
        Path(home_path)
        / "1-data"
    ...
    stable_path = (
        behavior_mouse_root
        / f"exp_config_nextsession_{exp_name}_{mouse_name}_latest.json"
    )

    if stable_path.is_file():
        return stable_path, stable_path

    filename_pattern = f"exp_config_nextsession_{exp_name}_{mouse_name}_*.json"
    search_roots = [
        # New unified behavior-data location.
        behavior_mouse_root,
        # Legacy purely-local sessions.
        Path(home_path) / "1-data" / "metadata",
        # Legacy temporary folders created by database-mode sessions.
        Path.home() / "Desktop" / exp_name / mouse_name,
    ]

    # Completed database-mode sessions are normally transferred here.
    if raw_data_dir:
        search_roots.append(Path(raw_data_dir) / exp_name / mouse_name)
    ...
    for root in search_roots:
        ...
        try:
            if root.exists():
                candidates.extend(root.rglob(filename_pattern))
        except OSError as e:
            print(f"[WARN] Could not search prior configs under {root}: {e}")
    ...
    if not valid_candidates:
        return None, stable_path

    latest_path = max(valid_candidates, key=lambda p: p.stat().st_mtime)
    return latest_path, stable_path
...
while True:
    choice_database = input("Do you want to use database? ([y] or [n]): ").strip().lower()

    if choice_database in {"y", "yes"}:
        USE_DATABASE = True
        break
    if choice_database in {"n", "no"}:
        USE_DATABASE = False
        break
    print("Please enter 'y or 'n")

exp_name             = 'pps_static_behavior_newHabituation'
home_path            = "Z:/projects/17. Goal-directed-PPS"
EXP_CONFIG_PATH      = os.path.join(home_path, f"3-config-json/exp_default/config_default_{exp_name}.json")
default_exp_config   = load_json(EXP_CONFIG_PATH)

# Read these sections in both modes. The path information is also useful when
# locating a prior session that was previously run with the database enabled.
db_conf    = default_exp_config.get("database", {})
paths_conf = default_exp_config.get("paths", {})

# All behavior sessions now use the same root, regardless of database mode.
behav_data_root = (
    Path(home_path)
    / "1-data"
    / "1-raw"
...
storage = SessionStorageManager(
    local_base_dir=behav_data_root,
    nas_base_dir=behav_data_root,
    smb_base_unc=str(behav_data_root),
)
...
if USE_DATABASE:
    ...
    datarecord = DataRecordCSV(behav_data_root)
...
# Preserve the base + rig + animal configuration. Choosing database parameters
# should not accidentally retain fields from the local previous-session update.
base_config_all = copy.deepcopy(config_all)

# =============================================================================
# Load parameters generated by the previous local session
# =============================================================================
# This stable path is independent of session_dir. That matters because database
# sessions write to a temporary folder, whereas purely local sessions write
# under home_path.
previous_local_config_path, LATEST_NEXT_CONFIG_PATH = find_latest_next_config(
    home_path=home_path,
    exp_name=exp_name,
    mouse_name=mouse_name,
    raw_data_dir=paths_conf.get("raw_data_dir"),
)

local_next_params = {}
if previous_local_config_path is not None:
    try:
        local_next_params = load_json(str(previous_local_config_path))
        if not isinstance(local_next_params, dict):
            raise ValueError("next-session config must contain a JSON object")

        config_all = deep_update(copy.deepcopy(base_config_all), local_next_params)
        print(f"[INFO] Loaded previous-session parameters from: {previous_local_config_path}")

        # Migrate a legacy session-specific file to the stable location so that
        # future starts do not need to search session folders.
        if previous_local_config_path != LATEST_NEXT_CONFIG_PATH:
            write_json_atomic(LATEST_NEXT_CONFIG_PATH, local_next_params)
            print(f"[INFO] Updated stable local config: {LATEST_NEXT_CONFIG_PATH}")
    except Exception as e:
        print(f"[WARN] Could not load local next-session config: {e}")
        local_next_params = {}
else:
    print("[INFO] No previous local next-session config found; using base/animal config.")

# =============================================================================
# Optionally replace the local previous-session update with database parameters
# =============================================================================
if USE_DATABASE:
    try:
        if mouse_id is None:
            raise DBClientError("Selected mouse has no database ID")

        new_params_database = mgr.get_next_params(
            mouse_id=mouse_id,
            experiment_id=db_conf["experiment_id"],
        ...
        while True:
            local_label = (
                str(previous_local_config_path)
                if previous_local_config_path is not None
                else "base/animal config (no prior local file found)"
            )
            config_source = input(
                "Use previous local parameters [L] or database parameters [D]? "
                f"\nLocal source: {local_label}\nChoice: "
            ).strip().lower()

            if config_source in {"l", "local"}:
                break
            if config_source in {"d", "database"}:
                config_all = deep_update(copy.deepcopy(base_config_all), new_params_database)
                print("[INFO] Applied next-session parameters from the database.")
                break
            print("Please enter 'L' or 'D'.")

    except Exception as e:
        print(f"[db] WARNING: could not retrieve next-session parameters: {e}")
        while True:
            use_local_config = input(
                "Continue using the local/base parameters? [y/n]: "
            ).strip().lower()
            if use_local_config in {"y", "yes"}:
                break
            if use_local_config in {"n", "no"}:
                sys.exit(1)
            print("Please enter 'y' or 'n'.")
...
TRIAL_TIME_TOLERANCE            = exp_conf["TRIAL_TIME_TOLERANCE"]  # In seconds
Y_DISTANCE_FROM_EDGE            = exp_conf["Y_DISTANCE_FROM_EDGE"]  # In center meters
BOTTOM_TOLANCE                  = exp_conf["BOTTOM_TOLANCE"]  
SPACE_DEGREES                   = exp_conf["SPACE_DEGREES"]
WARMUP_S                        = exp_conf["WARMUP_S"]
# ================================================================
# ball parameters
CIRCLE_RADIUS_CM                            =  ball_conf["CIRCLE_RADIUS_CM"]
BALL_FALL_SPEED_CM_S                         =  (Y_DISTANCE_FROM_EDGE -CIRCLE_RADIUS_CM)   / TRIAL_TIME_TOLERANCE
...
SUCCESS_EDGE_TOLERANCE_RANGE                =  [-CIRCLE_RADIUS_CM, CIRCLE_RADIUS_CM]
# BALL_OPACITY_DISTRIBUTION               = ball_conf["BALL_OPACITY_DISTRIBUTION"] 
BALL_OPACITY                                = ball_conf["BALL_OPACITY"]
...
SPAWN_Y_OFFSET              = SCREEN_HEIGHT_CM - Y_DISTANCE_FROM_EDGE
...
# ===============================================
# Create one unified session folder in both modes
# ===============================================
fallback_session_num = storage.get_next_session_num(
    mouse_name=mouse_name,
    experiment_name=exp_name,
)
ctx = None
...
if USE_DATABASE:
    ctx = mgr.begin_session(
        ...
        fallback_session_num=fallback_session_num,
        session_at=datetime.now(ZoneInfo("America/Chicago")),
    )
    session_dir = ctx.local_dir
else:
    session_dir, _, _, _ = storage.make_folders(
        mouse_name=mouse_name,
        experiment_name=exp_name,
        session_num=fallback_session_num,
        today=datetime.now(ZoneInfo("America/Chicago")).date(),
    )

os.makedirs(session_dir, exist_ok=True)
print(f"[INFO] Session data directory: {session_dir}", flush=True)


# # main sync log filename

SYNC_LOG_FILENAME = os.path.join(session_dir, f"sync_log_{exp_name}_{mouse_name}_{date_str}_{current_time}.csv")
ERROR_LOG_PATH = os.path.join(session_dir, 'error.txt')
EXP_CONFIG_FILENAME = os.path.join(session_dir, f"exp_config_{exp_name}_{mouse_name}_{date_str}_{current_time}.json")
PARAMS_FILENAME = os.path.join(session_dir, f"exp_params_{exp_name}_{mouse_name}_{date_str}_{current_time}.mat")

DISCONTINUOUS_CSV_FILENAME = os.path.join(session_dir, f"discontinuous_{exp_name}_{mouse_name}_{date_str}_{current_time}.csv")

# Session-specific copy for provenance. LATEST_NEXT_CONFIG_PATH is the stable
# cross-mode copy that the next run reads first.
NEXT_CONFIG_FILENAME = os.path.join(
    session_dir,
    f"exp_config_nextsession_{exp_name}_{mouse_name}_{date_str}_{current_time}.json",
)
BEHAV_RESULTS_FILENAME  =  os.path.join(session_dir, f"behav_results_{exp_name}_{mouse_name}_{date_str}.json")
...
class TrialConditionSampler:
    def __init__(self, chunk_size = 1500):
        ...
        ##### 2. radius of ball
        self.ball_radius_cm_list         = [CIRCLE_RADIUS_CM]
        ...
        self.ball_opacity_list             = [BALL_OPACITY]
        ...
        self.ball_y_speed_list            = [BALL_FALL_SPEED_CM_S]
       ...
       # self.no_random_walk_portion         = 1.0
        ...
        self.wheel_gain_list               = [WHEEL_GAIN_CM_PER_TICK]
       ...
       # self.no_wheel_jitter_portion       = 1.0
    ...
    ##### 1. initial x
        ...
        #### 5.  random walk of the ball
        new_random_walk_bias    = sample_from_list(self.chunk_size, [0.0])
        new_random_walk_std     = sample_from_list(self.chunk_size, [0.0])
        ...
        new_wheel_jitter    = sample_from_list(self.chunk_size, [0.0])
...
def spawn_ball_ts(now_ts, win_start, window_intervals, trial_params):
    ...
    ball = {
        ...
        'y_cm': half_screen_h - SPAWN_Y_OFFSET,
        'spawn_ts': now_ts,
...
try:
    ...
    while True:
        ...
        for bi, ball in enumerate(balls):
           ...
           # bottom_threshold = -half_screen_h + CIRCLE_RADIUS_CM
            ...
            bottom_tolerance_cm = -half_screen_h +ball['radius'] + BOTTOM_TOLANCE
           ...
           # if collided_with_obstruction or collided_with_screen_edge:
            ...
            if ball['y_cm'] <= bottom_threshold or (is_in_zone and ball['y_cm'] <= bottom_tolerance_cm):
                reach_bottom = 1
    ...
    [total_reward_amount, nRewarded_total, has_amount] = count_reward(SYNC_LOG_FILENAME)
    ...
    print(f"Reward amount free: {total_reward_amount - reward_amount_task}")
    print(f"Number of rewarded trials: {nReward_trials}")
    print(f"Number of free reward: {nRewarded_total - nReward_trials}")

    performance_results = analyze_pps_newHabituation(session_dir, prefix = exp_name)
    new_params          = update_params_pps_newHabituation(EXP_CONFIG_FILENAME, performance_results)


    performance_results["total_reward"]         = total_reward_amount
    performance_results["reward_amount_task"]   = reward_amount_task
    performance_results["nReward_trials"]       = nReward_trials
    with open(BEHAV_RESULTS_FILENAME, "w") as f:
        json.dump(performance_results, f, indent=2)
    
    # Keep one copy with the session and one stable per-mouse copy. The stable
    # copy survives database-mode transfer/deletion and is shared by both modes.
    write_json_atomic(NEXT_CONFIG_FILENAME, new_params)
    write_json_atomic(LATEST_NEXT_CONFIG_PATH, new_params)
    print(f"[INFO] Saved stable next-session config to: {LATEST_NEXT_CONFIG_PATH}")
