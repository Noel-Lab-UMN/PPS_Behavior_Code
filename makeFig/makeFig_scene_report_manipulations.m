...
save_name = fullfile(figure_folder,sprintf('%s_%s.svg', subjectCode, parameter_option));

switch subjectCode
    case 'LSZ_practice_5_violet'
        sgtitle_str = 'Example animal 1';
    case 'GD_1_red'
        sgtitle_str = 'GD_1_red';
end
...
metrics = {
    ...
    'zscore_p_reward',               'Z-score',                true;
    'target_distance_rewarded', 'Target distance',        false;
    'corr_trajectory_rewarded', 'Corr. trajectories', false;
...
for m = 1:size(metrics,1)
    ...
    if strcmp(metrics{m,1}, 'zscore_p_reward')
        line([condition_list(1), condition_list(end)], ...
             ...
             'LineStyle', '--', ...
             'Color', 'black');
        fprintf('For %s, %d/%d tested conditions is above chance level \n', ...
            parameter_option, sum(y_all > 1.645), numel(y_all));
    end
...
sgtitle(sgtitle_str, 'fontweight', 'bold', 'fontsize', 18, 'interpreter', 'none')
