...
for i_sub  = 1:numel(subjectCode_list)
    ...
    if ~isfolder(save_folder)
     ...
     for n = 1:numel(session_list)
        ...
        doTimebin = true;
        [percent_pps,percent_pps_timebin] = util_pps.get_probs_pps(behav_data, doTimebin, doPlot, EXP_CONFIG);
    
      
        %%% distance between the center and the ending position of target
        target_distance =  util_pps.compute_target_distance(behav_data);

        %%%% Percent of goal-directed, anti-goal-directed, and static
        movement_stats = util_pps.get_percent_movement(behav_data, doPlot, EXP_CONFIG);
    
        %%%%%% correlation between real and linear (ideal) trajectories
        correlation_stats = util_pps.get_trajectory_correlation_stats(behav_data);
        ...
        behav_results.target_distance       = target_distance; 
        behav_results.movement_stats          = movement_stats;
        behav_results.correlation_trajectory    = correlation_stats;
